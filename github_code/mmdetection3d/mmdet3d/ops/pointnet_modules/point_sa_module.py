...
class PointSAModuleMSG(nn.Module):
    ...
    def __init__(self,
        ...
        self.points_sampler = Points_Sampler(self.num_point, self.fps_mod_list,
                                             self.fps_sample_range_list) if self.num_point is not None else None
    ...
    def forward(
        ...
        if indices is not None:
            ...
            if features is None:
                assert self.use_xyz
                features_for_sample = xyz_flipped.clone().detach()
            else:
                features_for_sample = features
            indices = self.points_sampler(points_xyz, features_for_sample) if self.num_point is not None else None
            new_xyz = gather_points(xyz_flipped, indices).transpose(
                1, 2).contiguous() if self.num_point is not None else None

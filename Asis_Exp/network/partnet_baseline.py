...
def iou(pred, gt, gt_conf, nPoint, nMask):
    ...
    gt_x_matched = utils.gather_nd(gt, matching_idx_row).view(-1, nMask, nPoint)
    ...
    pred_x_matched = utils.gather_nd(pred, matching_idx_col).view(-1, nMask, nPoint)
...
def conf_loss(pred, gt_valid, iou_all, matching_idx):
    ...
    predicted_indices = utils.gather_nd(all_indeces, valid_idx)
    valid_iou = utils.gather_nd(iou_all, valid_idx)
